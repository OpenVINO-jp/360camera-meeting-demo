...
class omz_text_detection(ov_model):
    def run(self, ocvimg):
        ...
        #H, W, C = ocvimg.shape
        H,W = ocvimg.shape
##        rects = self.text_detection_postprocess(link, segm, (W,H), 0.9, 0.3) # segment,linkは要調整.0.95,0.30 の組合せは良い
...
### added by tk
##class omz_handwritten_japanese_recognition(ov_model):
##    def run(self, ocvimg):
##        return ""
...
### added by tk
##    def handwrittenJapaneseRecognition(self, omzmodel=None):
##        return self.modelObjectFactory('handwritten-japanese', omzmodel)
